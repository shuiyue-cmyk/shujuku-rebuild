...
import { assertSafeDirectApiBase, isDeepSeekFamilyModel, shouldInjectV4Instruction, shouldUseResponseFormat } from '../../src/service/biotracker/vendor/api.js';
import { buildWardrobePrepSystemPrompt, buildWardrobeStyleBookBlock, loadWardrobeStyleBook } from '../../src/service/biotracker/vendor/registry.js';
...
test('characters 用 null-proto：__proto__ 角色名不污染原型（安全审查 P2）', async () => {
  const { createEmptyChatState, getChatState } = await import('../../src/service/biotracker/vendor/state.js');
  const { applyToolCall } = await import('../../src/service/biotracker/vendor/tools.js');
  // 新状态 null-proto
